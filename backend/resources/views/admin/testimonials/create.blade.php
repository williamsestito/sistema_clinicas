...
<div class="p-6">
    <div class="flex items-center justify-between mb-4">
        <h1 class="text-xl font-semibold text-gray-700">Novo Depoimento</h1>
    ...
    <form action="{{ route('admin.testimonials.store') }}" method="POST" class="bg-white rounded-lg shadow p-6">
        @csrf
        ...
        <div class="mt-4">
            ...
            <textarea name="comment" id="comment" rows="4" required
                      class="wysiwyg w-full border rounded-md px-3 py-2 text-sm">{{ old('comment') }}</textarea>
        </div>
        <div class="mt-4">
            <label class="inline-flex items-center">
                <input type="hidden" name="visible" value="0">
                <input type="checkbox" name="visible" value="1" {{ old('visible', '1') == '1' ? 'checked' : '' }}
