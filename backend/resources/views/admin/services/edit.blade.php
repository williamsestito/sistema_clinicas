...
<div class="p-6">
  <div class="flex justify-between items-center mb-4">
    <h1 class="text-xl font-semibold text-gray-700">Editar Serviço</h1>
  ...
  <div class="bg-white rounded-lg shadow p-6">
    <form method="POST" action="{{ route('admin.services.update', $servico->id) }}" class="space-y-4">
      @csrf
      @method('PUT')
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="md:col-span-2">
          ...
          <input type="text" name="name" value="{{ old('name', $servico->name) }}" required
          ...
          <select name="professional_id" required class="w-full border rounded-md px-3 py-2 text-sm">
            ...
            @foreach($profissionais as $prof)
              <option value="{{ $prof->id }}"
                      {{ old('professional_id', $servico->professional_id) == $prof->id ? 'selected' : '' }}>
        ...
        <div>
          ...
          <input type="number" name="duration_min"
                 value="{{ old('duration_min', $servico->duration_min) }}"
        ...
        <div>
          ...
          <select name="active" class="w-full border rounded-md px-3 py-2 text-sm">
            <option value="1" {{ $servico->active ? 'selected' : '' }}>Ativo</option>
            <option value="0" {{ !$servico->active ? 'selected' : '' }}>Inativo</option>
          </select>
        </div>
        ...
        <div class="md:col-span-3">
          ...
          <textarea name="description" rows="3"
                    class="w-full border rounded-md px-3 py-2 text-sm">{{ old('description', $servico->description ?? '') }}</textarea>
        </div>
      </div>
